<template>
  <div>
    ...
    <el-row :gutter="20">
      <el-col :span="3.5">
        <div class="grid-content ">
          <el-input placeholder="请输入部门名称" v-model="selectData.deptName" clearable>
          </el-input>
        </div>
      </el-col>
    ...
    <!-- 表单 -->
    <el-table :data="tableData" stripe style="width: 100%" key="deptId" v-loading="loading">
      <el-table-column prop="deptId" label="id">
      </el-table-column>
      <el-table-column prop="deptName" label="部门名称">
      </el-table-column>
      <el-table-column prop="deptIntroduce" label="部门介绍">
      </el-table-column>
      
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primary" icon="el-icon-edit" circle @click="getOne(scope.row.deptId)"
          ...
          <el-popconfirm confirm-button-text='是的' cancel-button-text='不了' icon="el-icon-info" icon-color="red"
            @confirm='del(scope.row.deptId)' title="确定删除吗？">
    ...
    <el-dialog title="详情" :visible.sync="centerDialogVisible" width="50%" center>
      <el-form ref="form" :model="form" label-width="80px">
        <el-form-item label="部门id">
          <el-input v-model="form.deptId" :disabled="true"></el-input>
        </el-form-item>
        <el-form-item label="部门名称">
          <el-input v-model="form.deptName"></el-input>
        </el-form-item>
        <el-form-item label="部门介绍">
          <el-input type="textarea" v-model="form.deptIntroduce"></el-input>
        </el-form-item>
...
<script>
  export default {
    ...
    methods: {
      ...
      getList() {
        //axios
        var deptName = this.selectData.deptName;
        var deptIntroduce = this.selectData.deptIntroduce;
        var http = '/dept/' + this.pagination.currentPage
          + '/' + this.pagination.pageSize + '?q';
        if (deptName != null && deptName != '')
          http += '&deptName=' + deptName;
        if (deptIntroduce != null && deptIntroduce != '')
          http += '&deptIntroduce=' + depdeptIntroducetId;
        console.log(http)
      ...
      change() {
        ...
        this.$axios.put('/dept', this.form).then(res => {
          var r = res.data;
          if (!r.success) {
            this.wrong(r.errorMsg)
          } else {
      ...
      //删一个
      del(id) {
        this.$axios.delete('/dept/' + id).then(res => {
          var r = res.data;
          if (!r.success) {
            this.wrong(r.errorMsg)
          } else {
      ...
      //增一个
      add() {
        this.$axios.post('/dept', this.form).then(res => {
          var r = res.data;
          if (!r.success) {
            this.wrong(r.errorMsg)
          } else {
